...
//     if (req.user.role === "customer" && status.toLowerCase() !== "cancelled") {
//       return res
//         .status(400)
//         .json({ message: "Customers can only cancel appointments." });
//     }

//     // ✅ FIXED: Force lowercase so Mongoose validation never fails (e.g., "Completed" -> "completed")
...
//           message: `<p>Your appointment status has been updated to: <strong>${updated.status.toUpperCase()}</strong></p>`,
//         });
//       } catch (e) {
...
// // -------------------------------------------------------------------
// // 6. DELETE APPOINTMENT (Admin Clean-up & Doctor Deletion)
// // -------------------------------------------------------------------
...
//     // ✅ FIXED: Security Check - Allow Doctors to delete THEIR OWN appointments
...
//       // If they are not a doctor, they MUST be an admin to delete
//       return res
//         .status(403)
//         .json({
//           message:
//             "Not authorized. Only Admins and the assigned Doctor can delete.",
//         });
//     }
...
// // ===================================================================
// // 🩺 LOGIC FOR DOCTOR AND CUSTOMER DASHBOARDS
// // ===================================================================
...
//   try {
//     // Match the doctor by their EMAIL instead of userId,
...
//   try {
//     // Match the doctor by their EMAIL
//     const doctorProfile = await Doctor.findOne({
//       $or: [{ userId: req.user._id }, { email: req.user.email }],
//     });
...
//   getAvailability,
//   getAvailableDoctors,
//   getUserAppointments: getMyAppointments,
...
const createAppointment = async (req, res) => {
  try {
    ...
    // ✅ FIXED: Safely determine the Patient details.
    // If an Admin/Receptionist is booking this, it uses the provided body data.
    // If a normal user is booking, it uses their logged-in token data.
    const patientId = req.body.userId || req.body.patientId || req.user._id;
    const patientName = req.body.patientName || req.body.name || req.user.name;
    const patientEmail =
      req.body.patientEmail || req.body.email || req.user.email;
    const patientPhone =
      req.body.patientPhone || req.body.phone || req.user.phone || "N/A";
...
const deleteAppointment = async (req, res) => {
  try {
    ...
    // Security Check - Allow Doctors to delete THEIR OWN appointments
    if (req.user.role === "doctor") {
      const doctorProfile = await Doctor.findOne({
        $or: [{ userId: req.user._id }, { email: req.user.email }],
      });
...
const getDoctorAppointments = async (req, res) => {
  try {
    ...
    const formattedAppointments = appointments.map((app) => ({
      ...app.toObject(),
      // ✅ FIXED: Bulletproof fallback. If `app.user` is missing/corrupted, it pulls from `customerDetails`
      patient: app.user || app.customerDetails,
      time: app.timeSlot,
      reason: app.notes,
    }));
...
const getDoctorPatients = async (req, res) => {
  try {
    ...
    appointments.forEach((app) => {
      // ✅ FIXED: Safely grab patient data utilizing the fallback
      const patientData = app.user || app.customerDetails;

      if (patientData) {
        // Use email as a fallback key if _id is missing
        const uniqueKey = patientData._id?.toString() || patientData.email;

        if (!uniquePatientsMap.has(uniqueKey)) {
          uniquePatientsMap.set(uniqueKey, {
            _id: patientData._id || `guest-${Math.random()}`,
