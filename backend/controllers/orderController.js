        ...
        if (medicine.batches && medicine.batches.length > 0) {
          ...
          for (let i = 0; i < medicine.batches.length; i++) {
            ...
            if (medicine.batches[i].qty > 0) {
              // Take either the whole remaining requirement, or whatever is left in the batch
...
const updatePrescriptionStatus = asyncHandler(async (req, res) => {
  ...
  if (status === "Approved") {
    ...
    for (const item of order.orderItems) {
      ...
      if (medicine) {
        if (medicine.batches && medicine.batches.length > 0) {
          // Sort descending: Add stock back to the batch that expires LAST to maximize shelf life
          medicine.batches.sort((a, b) => {
            if (!a.expiryDate) return 1;
            if (!b.expiryDate) return -1;
            return new Date(b.expiryDate) - new Date(a.expiryDate);
          });
          medicine.batches[0].qty += item.qty;
        } else {
