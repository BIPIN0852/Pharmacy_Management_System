...
const drugData = {
  ...
  Drops: [
    ...
    "Vitamin D3 Drops (Kids)",
    "Multivitamin Drops (Kids)",
    "Colic Aid Drops",
    "Ondansetron Drops",
  ],
  Inhaler: [
  ...
  Surgical: [
    "Sterile Cotton Roll 500g",
    "Sterile Cotton Roll 100g",
    "Absorbent Gauze",
    "Roller Bandage 4 inch",
    "Roller Bandage 6 inch",
    "Crepe Bandage",
  ...
  categories.forEach((category) => {
    ...
    for (let i = 0; i < 50; i++) {
      ...
      let name =
        i < baseNames.length
          ? baseName
          : `${baseName} (Pack ${Math.floor(i / baseNames.length) + 1})`;
      ...
      allMedicines.push({
        ...
        prescriptionRequired:
          ["Injection", "Tablet", "Capsule"].includes(category) &&
          Math.random() > 0.5, // Randomly require Rx for some
      });
    }
  });
...
//     // Generate exactly 10 items per category
//     for (let i = 0; i < 10; i++) {
//       // Pick a base name. If we run out of unique names, append a variant number.
//       let baseName = baseNames[i % baseNames.length];
//       let name =
...
//     console.log(`✅ Successfully Imported ${medicines.length} Medicines!`);
//     console.log(`   (10 items for ${Object.keys(drugData).length} categories)`);
