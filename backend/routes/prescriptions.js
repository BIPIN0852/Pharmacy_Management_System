...
const express = require("express");
const Prescription = require("../models/Prescription"); // Ensure casing matches your file
const authenticateToken = require("../middleware/auth"); // Or '../middleware/authMiddleware' depending on your file
const multer = require("multer");
const fs = require("fs");
const path = require("path");
...
// ---------- Helper: Format Prescription for Frontend ----------
// Adds the full 'imageUrl' path so the frontend <img src> works
const formatPrescription = (doc) => {
  const p = doc.toObject ? doc.toObject() : doc;
  return {
    ...p,
    // This assumes this router is mounted at /api/customer/prescriptions
    imageUrl: `/customer/prescriptions/image/${p.imageFilename}`,
  };
};

// ---------- GET: Smart Route (Handles Admin vs Customer) ----------
// @route GET /api/customer/prescriptions
router.get("/", authenticateToken, async (req, res) => {
  try {
    let query = {};

    // 1. If it's a customer, only show THEIR prescriptions
    if (req.user.role === "customer") {
      query.customer = req.user.id;
    }
    // 2. If it's admin/pharmacist, allow seeing ALL (or keep specific logic)
    else if (["admin", "pharmacist"].includes(req.user.role)) {
      // Admins see all
      query = {};
    } else {
      return res.status(403).json({ message: "Access denied." });
    ...
    // Format data to include 'imageUrl'
    const formatted = prescriptions.map(formatPrescription);

    // Return object { prescriptions: [...] } to match Frontend expectation
    res.json({
      success: true,
...
// ---------- Serve prescription image safely ----------
// @route GET /api/customer/prescriptions/image/:filename
router.get("/image/:filename", (req, res) => {
  try {
    const filePath = path.join(uploadsDir, req.params.filename);
...
// ---------- Pharmacist dashboard route (Legacy/Specific) ----------
router.get("/pharmacist/prescriptions", authenticateToken, async (req, res) => {
  try {
    if (!["pharmacist", "admin"].includes(req.user.role)) {
      return res.status(403).json({ message: "Access denied" });
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      // If user didn't send name/email, try to fallback to logged in user data
      const finalName =
        customerName || (req.user.name ? req.user.name : "Unknown");
      const finalEmail =
        customerEmail || (req.user.email ? req.user.email : "No Email");
      ...
      // Return the formatted object so the frontend can add it to the list immediately
