...
const DoctorDashboard = () => {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        // 2. Generate Notifications (Added explicit 'action' routes)
        const alerts = pendingAppts.map((app) => ({
          id: app._id,
          type: "booking",
          title: "New Appointment Request",
          ...
          action: "/doctor/appointments", // FIXED: ensures onClick navigate works
        }));
        setNotifications(alerts);
  ...
  // --- Auto-open chat when navigated from message notification ---
  useEffect(() => {
    const openChatFromNotification = async () => {
      if (!location.state?.openChatForAppointment) return;
      
      const targetApptId = location.state.openChatForAppointment;
      
      // First try to find in already-loaded appointments
      let targetApp = recentAppointments.find((a) => a._id === targetApptId);
      
      // If not found in recent list, fetch from API
      if (!targetApp && !loading) {
        try {
          const res = await api.get("/doctor/appointments");
          const allAppts = res.data?.appointments || [];
          targetApp = allAppts.find((a) => a._id === targetApptId);
        } catch (err) {
          console.error("Failed to fetch appointment for chat:", err);
        }
      }
  ...
  // --- Chat Logic ---
  // This opens the modal instantly for the specific patient
  const handleOpenChat = async (app) => {
    setMessageTarget(app);
    setShowMessageModal(true);
    setChatLoading(true);
                  ...
                  {recentAppointments.map((app) => {
                    ...
                    return (
                      <li
                        ...
                        className="list-group-item p-4 d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3 hover-bg-light transition-all"
                      >
                        {/* Making the patient info clickable to open chat */}
                        <div
                          className={`d-flex align-items-center gap-3 ${!isPending ? "cursor-pointer" : ""}`}
                          onClick={() => {
                            if (!isPending) handleOpenChat(app);
                          }}
                        >
                          <div
                            className="bg-primary bg-opacity-10 text-primary rounded-circle d-flex align-items-center justify-content-center fw-bold shadow-sm flex-shrink-0"
        ...
        <div className="col-xl-4">
          ...
          >
            ...
            <div className="p-4 d-flex flex-column gap-3">
              ...
              {/* Added explicit Message Inbox link for the dashboard */}
              <button
                className="btn btn-outline-light rounded-pill py-2 text-start fw-bold d-flex align-items-center gap-3 border-opacity-25 hover-lift"
