...
const PharmacistInventory = () => {
  ...
  // Extracts the closest expiry date from the FEFO batches array
  ...
  const fetchMedicines = async () => {
    try {
      ...
      // Checks user role so it fetches full batch data without Access Denied errors
  ...
  const calculateStats = (data) => {
    ...
    //  Calculate expired using the exact batch expiry dates
  ...
  const filteredMedicines = medicines.filter((m) => {
    ...
    if (key === "alerts") {
      return (
        //  Filters alerts based on real expiry date
        matchSearch &&
