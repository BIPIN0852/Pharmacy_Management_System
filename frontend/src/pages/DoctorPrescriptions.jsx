...
const DoctorPrescriptions = () => {
  ...
  return (
    <div className="container-fluid py-4 px-3 px-md-4 bg-light min-vh-100 animate-fade-in">
      ...
      {/* 🚀 WRITE PRESCRIPTION MODAL (FIXED WITH REACT-BOOTSTRAP) */}
      <Modal
        show={showModal}
      ...
      >
        <Form onSubmit={handleSubmit}>
          <Modal.Header closeButton className="border-0 p-4 pb-3 bg-light">
            <Modal.Title className="fw-black mb-0 text-dark d-flex align-items-center gap-2 fs-5">
            ...
            {items.map((item, index) => (
              ...
              >
                <div className="col-md-4">
                  <label className="small mb-1 fw-medium">Medicine Name</label>
                  <input
          ...
          <Modal.Footer className="border-top border-light-subtle p-3 bg-light">
            <button
              type="button"
      ...
      {/* 🚀 PATIENT MEDICAL HISTORY MODAL (FIXED WITH REACT-BOOTSTRAP) */}
      <Modal
      ...
      >
        ...
        >
          <Modal.Title className="fw-black text-dark d-flex align-items-center gap-2 fs-5">
            <History className="text-primary" size={20} /> Medical History:{" "}
            {selectedAppt?.patient?.name}
          </Modal.Title>
        </Modal.Header>
        <Modal.Body
          className="p-4 bg-white custom-scrollbar"
                  ...
                  >
                          ...
                          <tbody className="small fw-medium text-dark">
                            {rx.items.map((item, idx) => (
                              <tr key={idx}>
                                ...
                                <td className="px-3">{item.durationDays}</td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
                    ...
                    {rx.notes && (
                      <div className="mt-3 bg-white p-3 rounded-3 small border border-light-subtle text-muted shadow-sm">
                        <strong className="text-dark">Notes:</strong> {rx.notes}
                      </div>
                    )}
                  </div>
                );
