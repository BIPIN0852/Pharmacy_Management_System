...
import { Link, useNavigate } from "react-router-dom";
import { useAuth } from "../context/AuthContext"; // ✅ Switched from Redux to Context
import api from "../services/api"; // ✅ Switched from raw Axios to your custom API service
import {
  Package,
  ChevronRight,
  ShoppingBag,
...
// Safely calculate the backend base URL for images
const API_BASE_URL =
...
const OrderHistory = () => {
  ...
  // ✅ Pull user from your AuthContext instead of Redux
  const { user } = useAuth();

  const fetchOrders = async () => {
    try {
      setLoading(true);
      // ✅ Use your custom 'api' instance. It automatically attaches the token!
  ...
  // ✅ Check for user from Context or a saved token in localStorage
  useEffect(() => {
