...
//     // Helper function to fetch order with a retry mechanism if payment is not yet marked true
//     const fetchOrderWithRetry = async (orderId, config, retries = 3) => {
...
//         // If it's COD, or if it's actually paid, return immediately!
//         if (res.data.isPaid || res.data.paymentMethod === "COD") {
//           return res;
//         }

//         // If it's not paid yet, wait 1.5 seconds and try again (giving backend time to sync)
//         if (i < retries - 1) {
//           await new Promise((resolve) => setTimeout(resolve, 1500));
//         } else {
//           // Return whatever we have on the final try
//           return res;
//         }
//       }
//     };
...
//                 // 4. Fetch the fully updated order using our Smart Retry function
...
//               console.error("Khalti Verification Error:", khaltiErr);
//               if (urlOrderId && isMounted) {
//                 const fallbackRes = await fetchOrderWithRetry(
//                   urlOrderId,
//                   config,
//                 );
//                 setOrder(fallbackRes.data);
...
//         // --- SCENARIO B: STRIPE, COD, OR DIRECT HISTORY VIEW ---
//         else if (urlOrderId) {
//           // Just fetching an existing order, but use retry in case Stripe webhook is slow
...
//   // FINANCIAL CALCULATIONS (Strict Formatting)
//   // ----------------------------------------------------
//   // 💥 THE FIX: Sometimes the backend takes an extra second. If we know this was a Khalti success (pidx exists & status is completed), force UI to show Paid.
//   const isPaid =
//     order.isPaid || (pidx && (khaltiStatus === "Completed" || !khaltiStatus));
//   const isCOD = order.paymentMethod === "COD";
