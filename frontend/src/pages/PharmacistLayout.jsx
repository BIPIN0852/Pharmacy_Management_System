...
const PharmacistLayout = () => {
  ...
  const fetchNotifications = async () => {
    try {
      ...
      // Safely extract data in case the backend wraps it in a nested data object
      const data = response.data?.data || response.data || {};
      const alerts = [];

      const ordersCount = data.pendingOrdersCount || 0;
      const rxCount = data.pendingPrescriptionsCount || 0;
      const stockCount = data.lowStockCount || 0;
      ...
      if (ordersCount > 0) {
        alerts.push({
          ...
          message: `${ordersCount} orders waiting for fulfillment.`,
      ...
      if (rxCount > 0) {
        alerts.push({
          ...
          message: `${rxCount} digital prescriptions need review.`,
      ...
      if (stockCount > 0) {
        alerts.push({
          ...
          message: `${stockCount} items have fallen below threshold.`,
      ...
      // 1. ALWAYS populate the notifications array so the dropdown has items
      setNotifications(alerts);

      // 2. Real-World Logic: Check if user manually cleared the badge for this specific number of alerts
      const dismissedCount = parseInt(
        sessionStorage.getItem("pharmacist_dismissed_count") || "0",
      );

      // 3. If a NEW alert arrived (alerts.length changed), or it hasn't been dismissed, show the red badge!
      if (alerts.length > 0 && alerts.length !== dismissedCount) {
        setUnreadCount(alerts.length);
      } else if (alerts.length === 0) {
        setUnreadCount(0);
        sessionStorage.removeItem("pharmacist_dismissed_count"); // Reset if queues are clear
      }
    } catch (err) {
  ...
  const handleClearNotifications = () => {
    // 1. Clear the red badge number on the bell
    setUnreadCount(0);

    // 2. Save the current number of alerts so the badge stays hidden... UNTIL a new order arrives!
    sessionStorage.setItem(
      "pharmacist_dismissed_count",
  ...
  const handleLogout = () => {
    ...
    // FORCE clear all lingering persistence to prevent phantom sessions
    localStorage.removeItem("token");
    localStorage.removeItem("userInfo");
    sessionStorage.removeItem("pharmacist_dismissed_count");
  ...
  return (
    ...
    >
            ...
            <div className="d-flex align-items-center gap-3 gap-md-4 flex-shrink-0">
              ...
              <Dropdown align="end">
                ...
                >
                  ...
                  {/* Badge only shows if there are UNREAD notifications */}
                  {unreadCount > 0 && (
                    <span
                ...
                >
                  <div className="bg-light p-3 border-bottom d-flex justify-content-between align-items-center">
                    ...
                    {/* The Header Badge always shows the total pending actionable items */}
                  ...
                  {/* "Mark as Read" only shows if the red badge is currently visible on the bell */}
                  {unreadCount > 0 && (
      ...
      <style>{`
        .pharmacist-layout-wrapper { position: relative; width: 100vw; }
        ...
        @media (max-width: 767.98px) {
          .sidebar-container { position: fixed; left: 0; top: 0; bottom: 0; overflow: hidden; z-index: 1050; }
          .sidebar-collapsed .sidebar-container { transform: translateX(-100%); }
          ...
          .sidebar-container .pro-sidebar-toggle { display: none !important; visibility: hidden !important; opacity: 0 !important; }
        }
