...
const DoctorAppointments = () => {
  ...
  const handleUpdateStatus = async (id, newStatus) => {
    try {
      ...
      // ✅ FIX: Force lowercase to match MongoDB strict Enums
      // (e.g., "Completed" becomes "completed")
                ...
                <Col md={6}>
                  <div className="bg-light p-3 rounded-3 border border-light-subtle h-100">
                    ...
                    <div className="d-flex align-items-center gap-3 mb-3">
                      ...
                      >
                        {getPatientInitials(selectedAppt.patient?.name)}
                      </div>
                      <div>
                        ...
                        <Badge bg="secondary" className="mt-1 fw-medium">
                          {selectedAppt.patient?.gender || "Patient"}
                        </Badge>
                      </div>
                    </div>
                    <div className="small text-muted mb-1">
                      <Mail size={14} className="me-2" />{" "}
                      {selectedAppt.patient?.email || "No Email"}
                    </div>
                    <div className="small text-muted">
                      <MapPin size={14} className="me-2" />{" "}
                      {selectedAppt.patient?.address || "Address not provided"}
                    </div>
                  </div>
                </Col>
